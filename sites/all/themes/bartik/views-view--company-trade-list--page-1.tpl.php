...
?>
	<script>
		...
		{
			var ajaxURL = '<?php global $base_url; echo $base_url; ?>' + '/company_dynamic_area/' + element.siblings(".trade_id").val();
		...
		jQuery(document).ready(function() {			
			...
			jQuery(".dynamic").delegate(".reply > div > .interview_not_pass_button", "click", function(){
			...
			jQuery(".dynamic").delegate(".reply > div > div > .yes_interview", "click", function(){
				...
				var ajaxURL = '<?php global $base_url; echo $base_url; ?>' + '/company_interview_not_pass/' + jQuery(this).parent().parent().siblings("input").val() + "/" + jQuery(this).siblings("div").children(".reject_note").val();
				jQuery(this).parent().parent().parent().parent(".dynamic").load(ajaxURL, function(responseText, textStatus, XMLHttpRequest) {
					if(textStatus == 'success') {
						reflash_dynamic_area(jQuery(this));
						reflash_trade_status_area(jQuery(this).siblings(".trade_status"));
			...
			jQuery(".dynamic").delegate(".reply > div > .offer_button", "click", function(){
				jQuery(".sub").hide();
				jQuery(this).siblings(".sub").show();
			});			
			jQuery(".dynamic").delegate(".reply > div > div > div > .offer_submit_cancel", "click", function(){
				jQuery(this).parent().parent().hide();
			});
			jQuery(".dynamic").delegate(".reply > div > div > div > .offer_submit", "click", function(){
				var root = jQuery(this);
				var offer_time = jQuery(this).parent().siblings().children(".time").val();
				var address = jQuery(this).parent().siblings().children(".address").val();
				var contact = jQuery(this).parent().siblings().children(".contact").val(); 
				var title = jQuery(this).parent().siblings().children(".title").val(); 
				var content = jQuery(this).parent().siblings().children(".content").val(); 
				if (!offer_time || !address || !contact || !title || !content)
				{
					alert('请填写完整后递交!');
					return;
				}
				var ajaxURL = '<?php global $base_url; echo $base_url; ?>' + '/send_offer/' + root.siblings(".trade_id").val() + '/' + offer_time + '/' + address + '/' + contact + '/' + title + '/' + content;
				jQuery.ajax({
				  url: ajaxURL,
				  success: function(data) {
				  	if (data == 'success')
				  	{
				  		alert('操作成功!');				   
						reflash_dynamic_area(root.parent().parent().parent().parent().parent());
						reflash_trade_status_area(root.parent().parent().parent().parent().parent().siblings(".trade_status"));
				  	}
				  	else
				  	{
				  		alert(data+',操作失败!');
			...
			jQuery(".dynamic").delegate(".reply > div > .on_position_confirm", "click", function(){
				var root = jQuery(this);
				if (confirm("确定此人到岗么?"))
				{
					var ajaxURL = '<?php global $base_url; echo $base_url; ?>' + '/on_position_confirm/' + jQuery(this).parent().parent().parent().siblings(".trade_id").val();
					jQuery.ajax({
					  url: ajaxURL,
					  success: function(data) {
					  	if (data == 'success')
					  	{
					  		alert('操作成功!');				   
							reflash_dynamic_area(root.parent().parent().parent());
							reflash_trade_status_area(root.parent().parent().parent().siblings(".trade_status"));
					  	}
					  	else
					  	{
					  		alert(data+',操作失败!');
			...
			jQuery(".dynamic").delegate(".reply > div > .on_position_not_confirm", "click", function(){
				var root = jQuery(this);
				if (confirm("确定此人没有到岗么?"))
				{
					var ajaxURL = '<?php global $base_url; echo $base_url; ?>' + '/on_position_not_confirm/' + jQuery(this).parent().parent().parent().siblings(".trade_id").val();
					jQuery.ajax({
					  url: ajaxURL,
					  success: function(data) {
					  	if (data == 'success')
