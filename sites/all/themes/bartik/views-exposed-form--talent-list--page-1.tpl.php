...
<script>
	jQuery(document).ready(function()
	{
		...
		jQuery("#edit-sort-order").change(function() {
  			jQuery("#views-exposed-form-talent-list-page-1").submit();
		});
...
<div id="cw_filter"> 
	...
	<?php if ($id == 'filter-field_talent_height_value' || $id == 'filter-title' || $id == 'filter-status') continue; ?>
	<div>
		<div class="label inline_block">
	...
	<div>
		...
		<div class="content inline_block">
			<input type="text" id="edit-field-talent-height-value-min" name="field_talent_height_value[min]" value="<?php if (isset($_GET['field_talent_height_value']['min'])) {echo $_GET['field_talent_height_value']['min'];} ?>" size="3" maxlength="3" class="form-text" />&nbsp;CM
			-
			<input type="text" id="edit-field-talent-height-value-max" name="field_talent_height_value[max]" value="<?php if (isset($_GET['field_talent_height_value']['max'])) {echo $_GET['field_talent_height_value']['max'];} ?>" size="3" maxlength="3" class="form-text" />&nbsp;CM
		</div>
	</div>
</div>
<hr>
<div>
	<div class="inline_block" style="width:115px">人才姓名</div>
	<div class="content inline_block" style="width:115px">
		<?php echo str_replace(array('是', '否'), array('已发布', '未发布'), get_select($widgets['filter-status']->widget) ); ?>
	</div>
	<div class="inline_block" style="width:140px; padding-left: 70px">人才信息</div>
	<?php if (!empty($sort_by)): ?>
		...
		<div class="form-item form-type-select form-item-sort-order inline_block">
			<select id="edit-sort-order" class="form-select" name="sort_order" style="width:120px">
				<option <?php if (isset($_GET['sort_order']) && $_GET['sort_order'] == 'ASC') echo 'selected="selected"'; ?> value="ASC">更新时间由远到近</option>
				<option <?php if (!isset($_GET['sort_order']) || (isset($_GET['sort_order']) && $_GET['sort_order'] == 'DESC')) echo 'selected="selected"'; ?> value="DESC">更新时间由近到远</option>
			</select>
		</div>
    <?php endif; ?>
